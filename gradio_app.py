...
def gen_multiview(pipeline, predictor, input_image, scale_slider, steps_slider, seed, output_processing=False):
    ...
    if not os.path.exists("sides_view"):
        os.makedirs("sides_view")
    if "Background Removal" in output_processing:
        ...
        for i, sub_image in enumerate(subimages):
            ...
            # save the image to path "sides_view/image_{i}.png"
            sub_image.save(f"sides_view/image_{i}.png")
        return out_images
    # save the subimages to path "sides_view/image_{i}.png"
    # save the subimages
    for i, sub_image in enumerate(subimages):
        sub_image.save(f"sides_view/image_{i}.png")
    return subimages
